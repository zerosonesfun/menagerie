 ...
 * @package Menagerie
 */
import { encodeInWorker } from './wasm-bridge.js';
...
(function () {
	...
	/**
	 * Try jSquash WASM in Worker (MozJPEG / WebP / AVIF). Returns null to use native canvas path.
	 *
	...
	function tryWasmEncode(canvasEl, mime) {
		...
		var id;
		try {
			id = ctx.getImageData(0, 0, canvasEl.width, canvasEl.height);
		} catch (e) {
			return Promise.resolve(null);
		}
		var qp = getQualityPercent();
		return encodeInWorker(id, mime, qp).then(function (ab) {
			if (!ab) {
				return null;
			}
			return { blob: new Blob([ab], { type: mime }), mime: mime };
		}).catch(function () {
			return null;
		});
	}
